...
<body>
				...
				<ul class="nav navbar-nav">
					...
					<li class="dropdown"><a href="#" class="dropdown-toggle"
						...
						<ul class="dropdown-menu" role="menu">
							...
							<li role="presentation" class = "active"><a href="viewRoute">View Route</a></li>
						</ul></li>
				</ul>
								...
								<c:forEach var="row" items="${routeList}">
									<tr>
										...
										<td><input type="button"
											onclick="editSetRoute(${row.id })" value="View"
											style="color: green" />&emsp;<input type="button"
											onclick="deleteSetRoute(${row.id })" value="Delete"
											style="color: red" /></td>
									</tr>
								</c:forEach>
							</tbody>
	...
	<!-- Wrapper -->
	<script type="text/javascript">
		function editSetRoute(id){
			location.href = "${pageContext.request.contextPath}/" + id + "/editSr";
		}
	
		function deleteSetRoute(id){
			var r = confirm("Are you sure you want to delete this recored?");
			if (r == true) {
				window.location = "${pageContext.request.contextPath}/" + id + "/deleteSr";
			} 
		}
